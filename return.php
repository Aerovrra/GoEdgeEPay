...
if (validateSign($_GET, $config['api_key'])) {
    if ($trade_status == 'TRADE_SUCCESS') {
        // 处理支付成功的逻辑
        // 更新订单状态等
        echo '支付成功 请返回网站查看'; // 返回success表示接收成功
        //当然这里你自己也可以随便发挥
    } else {
        echo 'fail'; // 处理失败
    }
...
function set_order_success($trade_no){
    ...
    $access_token_json = sendPostJson($config['api_url']."/APIAccessTokenService/getAPIAccessToken",json_encode($auth_info,true),"none");
    $access_token = json_decode($access_token_json,true);
    $token = $access_token['data']['token'];
    $code = [
        "code"=>$trade_no
    ];
    $finish_order = sendPostJson($config['api_url']."/UserOrderService/finishUserOrder",json_encode($code,true),$token);
    return $finish_order;
}
